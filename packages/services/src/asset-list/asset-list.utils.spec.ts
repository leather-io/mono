...
describe(filterAssetsByProtocol.name, () => {
  test('filters to requested protocols', () => {
    const result = allAssets.filter(filterAssetsByProtocol(['nativeBtc', 'sip10']));
    expect(result).toHaveLength(2);
    expect(result.map(a => a.protocol)).toEqual(['nativeBtc', 'sip10']);
  });

  test('returns empty when no protocols match', () => {
    const result = allAssets.filter(filterAssetsByProtocol([]));
