...
import { getTaprootAddressIndexDerivationPath } from '@leather-wallet/bitcoin/src/p2tr-address-gen';
import { getNativeSegwitAddressIndexDerivationPath } from '@leather-wallet/bitcoin/src/p2wpkh-address-gen';
...
export interface Signer<Payment> {
  ...
  signIndex(tx: btc.Transaction, index: number, allowedSighash?: btc.SigHash[]): void;
}
...
interface MakeBitcoinSignerArgs {
  ...
  signAtIndexFn(tx: btc.Transaction, index: number, allowedSighash?: btc.SigHash[]): void;
}
