...
describe(deriveKeychainDescriptor.name, () => {
  test('it derives the correct account descriptor', async () => {
    ...
    expect(deriveKeychainDescriptor(keychain, "m/84'/0'/0'")).toEqual(
    ...
    expect(deriveKeychainDescriptor(keychain, "m/84'/0'/1'")).toEqual(
  ...
  test('it throws an error when deriving from non-root keychain', async () => {
    const keychain = deriveRootBip32Keychain(await deriveBip39SeedFromMnemonic(mnemonic));
    expect(() => deriveKeychainDescriptor(keychain.derive("m/84'/0'/0'"), "m/84'/0'/0'")).toThrow();
  });
});
