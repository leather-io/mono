...
import { getAssetId, serializeAssetId, type SerializedCryptoAssetId } from '@leather.io/utils';
...
export function filterActivityBySerializedAssetId(
  ...
  return activity.filter(view =>
    getActivityViewAssetIds(view).includes(serializedAssetId)
  );
}
