...
internal class SecAccessControlError: GenericException<Int?> {
    override var reason: String {
        return
            "Unable to construct SecAccessControl: \(param.map { "code " + String($0) } ?? "unknown error")"
    }
}
...
internal class KeyChainException: GenericException<OSStatus> {
    override var reason: String {
        ...
        default:
            if let errorMessage = SecCopyErrorMessageString(param, nil)
                as? String
            {
                return errorMessage
            }
...
internal class LocalAuthenticationException: GenericException<NSError> {
    override var reason: String {
        switch param.code {
        ...
        default:
            return "unknown: \(param.code), \(param.localizedDescription)"
