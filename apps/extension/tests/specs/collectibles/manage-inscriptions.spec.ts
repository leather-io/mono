...
test.describe('Manage inscriptions', () => {
  ...
  test.describe('manage inscriptions sheet', () => {
    ...
    test('should display ordinals protection toggle', async ({ settingsPage, page }) => {
      await settingsPage.clickManageInscriptions();

      const toggle = page.getByTestId(SettingsSelectors.OrdinalsProtectionToggle);
      await expect(toggle).toBeVisible();
    });

    test('should display unprotect all button', async ({ settingsPage, page }) => {
      await settingsPage.clickManageInscriptions();

      const button = page.getByTestId(SettingsSelectors.UnprotectAllInscriptionsBtn);
      await expect(button).toBeVisible();
    });

    test('should toggle ordinals protection', async ({ settingsPage, page }) => {
      await settingsPage.clickManageInscriptions();
  ...
  test.describe('protection state persistence', () => {
    ...
    test('should persist protection state after closing sheet', async ({
      page,
      networkPage,
      homePage,
