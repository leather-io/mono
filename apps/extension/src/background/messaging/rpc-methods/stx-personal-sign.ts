...
export const stxPersonalSignHandler = defineRpcRequestHandler(
  stxPersonalSign.method,
  async (request, port) => {
    ...
    const { id } = await triggerRequestPopupWindowOpen(RouteUrls.RpcPersonalSign, urlParams);
