...
export function InscriptionDetailsPage({ view, onBack }: InscriptionDetailsPageProps) {
  ...
  const handleToggleProtection = useCallback(() => {
    if (isDiscarded) {
      recoverInscriptions(inscription);
    } else {
      discardInscriptions(inscription);
    }
  }, [inscription, isDiscarded, recoverInscriptions, discardInscriptions]);
  ...
  return (
    <Stack width="100%" gap="space.00" data-testid="collectible-details-container">
      ...
      <CollectibleDetailsPageLayout
        ...
        media={<InscriptionCard item={inscription} />}
      >
        <InscriptionDetails
          asset={inscription}
