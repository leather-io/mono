...
export function SignOutSheet({ isShowing, onUserDeleteWallet, onClose }: SignOutSheetProps) {
  const hasSoftwareKeys = useSelector(selectSoftwareKeys).length > 0;

  const form = useFormik({
    initialValues: {
      confirmBackup: !hasSoftwareKeys,
    ...
    >
      ...
      <Flex alignItems="center" flexDirection="column" p="space.05">
        <form onChange={form.handleChange} onSubmit={form.handleSubmit}>
          <styled.label alignItems="center" display={hasSoftwareKeys ? 'flex' : 'none'}>
            <HStack gap="space.03">
              <input
                type="checkbox"
                name="confirmBackup"
