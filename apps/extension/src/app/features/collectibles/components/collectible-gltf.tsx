 ...
 */
export function CollectibleGltf({ src, thumbnailSrc, onPress }: CollectibleGltfProps) {
  ...
  if (onPress) {
    return (
      <CollectibleImage alt="Collectible preview" src={thumbnailSrc ?? src} onPress={onPress} />
    );
  }

  return (
    <CollectibleCard>
      <Iframe src={dataUrl} height="100%" width="100%" onError={() => setHasError(true)} />
    </CollectibleCard>
  );
}
