...
export function Sip9TokenDetails({ asset }: Sip9TokenDetailsProps) {
  ...
  const { name, description, tokenId, collection } = asset;
    ...
    <Collectible name={name} details={asset}>
      ...
      <TokenDetailsCard title={t`Collectible Info`}>
        <SummaryTableRoot>
          ...
          <SummaryTableItem
            label={t`Collection`}
            value={<DetailsLink url={collectionLink} label={collectionName ?? ''} />}
          />
          <SummaryTableItem label={t`Creator`} value="" />
          {totalItems && <SummaryTableItem label={t`Rarity rank`} value={t`?? of ${totalItems}`} />}
