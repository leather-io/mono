...
export function Sip9TokenDetails({ asset }: Sip9TokenDetailsProps) {
  ...
  const hiroExplorerContractUrl = useGetHiroExplorerUrl({ type: 'address', value: asset?.contractId ?? '' });
      ...
      <TokenDetailsCard>
        ...
        <TokenDetailsCard title={t`Collectible Info`}>
          <SummaryTableRoot>
            ...
            <SummaryTableItem label={t`Collection`} value={<DetailsLink url={collectionLink} label={collection?.name ?? ''} />} />
            <SummaryTableItem label={t`Creator`} value='' />
            <SummaryTableItem label={t`Rarity rank`} value={t`?? of ${collection?.totalItems ?? ''}`} />
            ...
            <SummaryTableItem label={t`Contract`} value={<DetailsLink url={hiroExplorerContractUrl} label={truncateMiddle(asset?.contractId ?? '', 5)} />} />
...
{asset?.attributes && asset?.attributes?.length > 0 && (
    <TokenDetailsCard title={t`Attributes`}>
      <SummaryTableRoot>
        {asset?.attributes?.filter((attribute) => attribute?.traitType && attribute?.value && attribute?.value !== 'None').map((attribute) => {
             const attributeValue = attribute?.rarityPercent
          ? `${attribute?.value} (${attribute?.rarityPercent}%)`
          : String(attribute?.value);
          return (
            <SummaryTableItem key={attribute?.traitType} label={attribute?.traitType} value={attributeValue} />
          )
        })}
      </SummaryTableRoot>
    </TokenDetailsCard>
