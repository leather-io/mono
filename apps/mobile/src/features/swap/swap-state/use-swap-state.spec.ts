...
describe('useSwapState', () => {
  ...
  describe('quote enrichment and selection', () => {
    it('initializes with quoteStrategy set to best', () => {
      const result = renderUseSwapState();
      expect(result.current.state.quoteStrategy).toBe('best');
    });
  ...
  describe('network fees', () => {
    ...
    describe('fee query lifecycle', () => {
      ...
      it('networkFeeQuery refetches when dependencies change', async () => {
        ...
        const secondData = result.current.networkFeeQuery.data;

        act(() => result.current.actions.setSlippage(0.05));
        await waitFor(() => {
          expect(result.current.networkFeeQuery.data).not.toBe(secondData);
        });
      });
    });
