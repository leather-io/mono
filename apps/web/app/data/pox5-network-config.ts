...
type Pox5ChainName = 'mainnet' | 'devnet' | 'stakingTestnet';
...
interface Pox5ChainConfig {
  ...
  // ids resolve from. The staking testnet is testnet-flavoured (ST addresses),
...
// wallet setup is needed. stakingTestnet is Hiro's hosted pox-5 staking
// testnet, which Leather ships as the built-in `stakingTestnet` network, so no
// wallet setup is needed either.
const pox5Chains: Record<Pox5ChainName, Pox5ChainConfig> = {
  ...
  stakingTestnet: {
    walletRpcNetwork: WalletDefaultNetworkConfigurationIds.stakingTestnet,
    contractNetworkMode: 'testnet',
    apiUrl: HIRO_API_BASE_URL_STAKING_TESTNET,
    stacksNetworkName: 'testnet',
    bitcoinNetworkMode: 'signet',
  },
};
