...
import { useLisaBalance } from '~/queries/protocols/lisa/balance';
import { useStackingTrackerProtocol } from '~/queries/stacking-tracker/protocols';

export function useLisaInfo() {
  const protocolSlug: ProtocolSlug = 'lisa';
  const providerId: ProviderId = 'lisa';

  const poxInfoQuery = useGetPoxInfoQuery();
  const balance = useLisaBalance();
