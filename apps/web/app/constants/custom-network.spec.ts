...
async function loadCustomNetwork() {
  vi.resetModules();
  return import('./custom-network');
}
...
describe('customNetwork', () => {
  test('is null when no private-network env vars are set', async () => {
    const { customNetwork } = await loadCustomNetwork();
    expect(customNetwork).toBeNull();
  });

  test('resolves when flavor + both api urls are set', async () => {
  ...
  test('is null when flavor is set but api urls are missing', async () => {
    vi.stubEnv('LEATHER_PRIVATE_NETWORK_FLAVOR', 'regtest');
    const { customNetwork } = await loadCustomNetwork();
    expect(customNetwork).toBeNull();
  });
  ...
  test('defaults name/key and leaves chain-id undefined when unset', async () => {
    ...
    stubUrls();
    const { customNetwork } = await loadCustomNetwork();
    expect(customNetwork?.name).toBe('Private');
    expect(customNetwork?.key).toBe('private');
    expect(customNetwork?.stacksChainId).toBeUndefined();
  });
});
