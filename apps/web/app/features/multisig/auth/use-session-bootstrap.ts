...
const restoringSessionsAtom = atom<ChainNetworkId[]>([]);

export function useIsRestoringSession(network: ChainNetworkId) {
  return useAtomValue(restoringSessionsAtom).includes(network);
}

async function restoreSession(network: ChainNetworkId, session: AuthSession) {
