...
test.describe('Bitcoin Staking', () => {
  test('users can start staking with Fast Pool', async ({ page, mode }) => {
    ...
    await page.getByTestId('start-staking-button-fast-pool').click();
    ...
    await page.waitForURL('**/staking/pool/fast-pool/active', { timeout: 15_000 });
  ...
  test('already-staked users are redirected to their active position', async ({ page, mode }) => {
    ...
    await page.waitForLoadState('networkidle');
    await page.goto('/staking/pool/fast-pool');
